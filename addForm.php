...
if (count($_POST) > 0) {
    if(isset($_SESSION['id'])) {
        ...
        if (!is_numeric($insPrice)){
            $errors .= "Invalid price";
        }
        $insCategory = $_POST['insCategory'] ?? '';
        $total = $insPrice * $quantity;
        $query = "INSERT INTO forms(formEmail,formPhone,formAddress,formFirstName,formLastName,formQuantity,formMessage,userId,insId) values(?,?,?,?,?,?,?,?,?) ";
